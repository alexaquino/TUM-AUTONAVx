...
def euler_angles_to_rotation_matrix(phi, theta, psi, is_degree = True):
    ...
    X = np.array([[1, 0, 0], [0, cos(phi), -1 * sin(phi)], [0, sin(phi), cos(phi)]]);
    Y = np.array([[cos(theta), 0, sin(theta)], [0, 1, 0], [-1 * sin(theta), 0, cos(theta)]]);
    Z = np.array([[cos(psi), -1 * sin(psi), 0], [sin(psi), cos(psi), 0], [0, 0, 1]]);
